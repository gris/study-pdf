...
export async function addHighlightAnnotation(
	...
	const { pageIndex, quadPoints, box, color, opacity = DEFAULT_OPACITY } = options;
...
/** Reads a highlight annotation's /Contents string. Handles both string
 * encodings a real-world PDF may use (literal PDFString and PDFHexString). */
function readNote(dict: PDFDict): string | null {
	const contents = dict.get(PDFName.of('Contents'));
	const text = (contents as { decodeText?: () => string } | undefined)?.decodeText?.();
	return text || null;
}
